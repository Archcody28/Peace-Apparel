...
export default async function handler(req, res) {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, x-admin-registration-secret');
  ...
  try {
    ...
    if (expectedSecret && providedSecret !== expectedSecret) {
      return res.status(401).json({ error: 'Unauthorized registration request' });
    }
    ...
    try {
      ...
      return res.status(201).json({
        message: 'Admin registered successfully (local fallback)',
        token: fallback.token,
        user: fallback.user
      });
    }
  } catch (err) {
    console.error('Admin registration API error:', err);

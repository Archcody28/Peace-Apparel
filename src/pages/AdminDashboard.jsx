...
import {
  ...
  Check, ChevronDown, Sun, Moon, Command, TrendingUp, DollarSign,
  ShoppingBag, UserCheck, Loader2
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const [p, o, s, t, h, a, st] = await Promise.all([
        fetch(base + '/api/products?limit=200').then(r => r.json()),
        fetch(base + '/api/orders').then(r => r.json()),
        fetch(base + '/api/subscribers').then(r => r.json()),
        fetch(base + '/api/testimonials').then(r => r.json()),
        fetch(base + '/api/homepage-features').then(r => r.json()),
  ...
  const deleteProduct = async (id) => {
    ...
    await fetch(base + '/api/products', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
    fetchAll();
  };

  const updateOrderStatus = async (id, status) => {
    await fetch(base + '/api/orders', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, status }) });
    fetchAll();
  };

  const deleteOrder = async (id) => {
    if (!confirm('Delete this order?')) return;
    await fetch(base + '/api/orders', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
    fetchAll();
  };
  ...
  const saveTestimonial = async (e) => {
    ...
    await fetch(base + '/api/testimonials', { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(testimonial) });
  ...
  const deleteTestimonial = async (id) => {
    ...
    await fetch(base + '/api/testimonials', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
    fetchAll();
  };
  ...
  const saveFeature = async (e) => {
    ...
    await fetch(base + '/api/homepage-features', { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(feature) });
  ...
  const deleteFeature = async (id) => {
    ...
    await fetch(base + '/api/homepage-features', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
    fetchAll();
  };
  ...
  const saveSettings = async (e) => {
    ...
    await fetch(base + '/api/settings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newSettings) });
    setSettings(newSettings);
  };

  const deleteSubscriber = async (id) => {
    if (!confirm('Delete this subscriber?')) return;
    await fetch(base + '/api/subscribers', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
    fetchAll();
  };
